...
| **`trivy_scan`** | CVE scanning of container images via Trivy, ephemeral container routed through the maintenance proxy, cached DB+per-image analysis in a named volume (`trivy-cache`), sequential (not parallel) scanning | 2026-08-18 |
...
## 🔍 `trivy_scan` — technical details (2026-08-18)

### Architecture

Ephemeral Trivy container (`docker run --rm`) per scan, routed through `docker_env("maintenance")` — same proxy as sandbox/permafrost, since it needs `docker run` (container-create), not just read access. Unlike the sandbox, it **does** need network access (fetches the CVE database), so it does not get `--network=none`.

CVE database + per-image analysis cache persist in a named volume (`trivy-cache`), mounted into every ephemeral container. With no target parameter, it scans every currently running container (discovered dynamically via `docker ps`, never hardcoded — same pattern as the volume discovery in `protocol_permafrost`).

### Bugs found + fixed

- **🔴 Parallel scanning deadlock** (serious): First implementation ran all containers **in parallel** (`ThreadPoolExecutor`) for speed. Result: 13/16 scans failed with `"cache may be in use by another process: timeout"` — Trivy's on-disk cache (BoltDB) doesn't support concurrent writers. Fix: sequential scanning instead of parallel. With an already-cached DB, each scan takes ~0.75-1 second, so sequential stays fast enough (16 containers in ~12 seconds) — the parallelism wasn't worth the complexity of a lock/semaphore mechanism.
- **Orphaned container held a stale lock**: After the first failed parallel run, a Trivy container (`unruffled_cohen`) stayed `Up` for 14+ minutes instead of exiting via `--rm` — it held the lock on the `trivy-cache` volume, blocking **every** subsequent attempt (even sequential, even single-target). `docker volume rm` failed with a quiet informational message ("volume is in use") instead of erroring loudly — that was the real diagnostic clue. Fix: `docker rm -f` the orphaned container first, then `docker volume rm` + `docker volume create` for a clean restart. General lesson: `docker ps -a --filter "volume=<name>"` is the first check whenever a volume "won't clean up."

### Performance characteristics (measured, not theoretical)

- **First-ever full scan (16 containers, cold DB + cold per-image cache)**: ~6m35s
- **Full scan with an already-warm cache**: ~12s
- Single-target scan with a warm cache: <1s

The big gap (6m35s → 12s) isn't just the one-time DB download (~108MB) — the per-image analysis cache is built separately for each image the first time it's scanned, not just the CVE DB once overall.

### Practical finding from the first real scan

Vaultwarden: 8 critical, 39 high, 92 medium, 28 fixable. Grafana: 9 critical, 107 high, 165 medium, 281 fixable. Open WebUI: 27 critical, 460 high, 2045 medium (confirmed standalone, not a parsing bug — large dependency tree, Debian+Python+Node+Rust combined). Pi-hole: 0 critical, 15 high, 4 medium, 19 fixable — all in `bind-libs`/`bind-tools`/`c-ares`.

Tried `docker pull` + a Portainer stack redeploy on Vaultwarden (expecting it to fix CVEs) — **no change** in the numbers afterward. Explanation: the `curl`/`gzip` CVEs on the Vaultwarden image don't have a fixed version upstream yet (`fixed: null` in the raw Trivy output) — not something pull/redeploy can fix, it's upstream-blocked until the next base image release. Clean reminder: "fixable" in the tool means "a fixed version exists," not "will be fixed if you pull right now" — if the image you're pulling already contains the fix, pulling again changes nothing.
